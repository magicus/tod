...
public class ConnectionManager
{
	...
	public synchronized void connect()
	{
		...
		try
		{
			itsConfig.log(1, "Connecting to "+itsConfig.getHost()+":"+itsConfig.getNativePort());
			theSocket = new Socket(itsConfig.getHost(), itsConfig.getNativePort());
			in = new DataInputStream(theSocket.getInputStream());
			out = new DataOutputStream(theSocket.getOutputStream());
		}
		catch (IOException e)
		{
			System.out.println("Could not connect to "+itsConfig.getHost()+":"+itsConfig.getNativePort()+", aborting.");
			System.exit(1);
		}
		...
		try
		{
			...
			while(! theDone)
			{
				...
				case NativeAgentPeer.SET_WORKING_SET:
					itsConfig.setWorkingSet(in.readUTF());
