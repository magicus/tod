...
Si el programa imprime \texit{la respuesta es 41} en vez de \texit{42}, entonces nosotros tenemos un manejador sobre el bug.  Nosotros vemos la serpiente en el pasto y podemos agarrar su cola.  Ahora si el programa falla al producir la respuesta correcta, entonces nosotros no tenemos un manejador directo del bug.  Esta es la serpiente en el pasto que no podemos ver.
...
Con ODB, existe una versión de este problema.  Cuando la producción incorrecta es causada por la falta de un determinado objeto cuando esta siendo asignado, entonces nosotros tenemos que responder la pregunta \textit{¿Quién pudo haber echo esto?}, la cual es una pregunta mucho más dificil que \texit{¿Quién hizo esto?} (Esta es el lagarto sin pata en el problema del pasto).  Nosotros pensamos que tenemos una serpiente, pero despues de tirar tu cola por un momento, esta la quiebra de cualquier forma.
...
\begin{description}
	...
	\itema[Soporte para huellas parciales] Ofreciendo mecanismos estáticos y dinámicos para la generación selectiva de la huella y un reporte adecuado para la información incompleta.
