...
/**
 * This is a test case for measuring the performance of the database.
 * This runs quite short without TOD but generates about 130M events with TOD.
 * @author gpothier
 */
public class BurnMasterBench
{
...
//		theStructureDatabase.clear();
		...
		System.out.println("setting up structure database...");
...
//			theClass.getNewField("f"+i, PrimitiveTypeInfo.BOOLEAN, false);
		...
		for (int i=0;i<7;i++)
		{
			...
			int theCount = checkCondition(theMaster, theEventCondition, createEventGenerator(theStructureDatabase), 1000, 3000);
			
			GridEventBrowser theEventBrowser = new GridEventBrowser(theLogBrowser, theEventCondition);
			int theCount2 = checkCondition(theEventBrowser, theEventCondition, createEventGenerator(theStructureDatabase), 1000, 3000);
	...
	{
		System.out.println("BurnMasterBench.main()");
		long t0 = System.currentTimeMillis();
		new BurnMasterBench().test();
		long t1 = System.currentTimeMillis();
		Utils.println("BurnMasterBench - done in %d seconds.", (t1-t0)/1000);
		System.exit(0);
	}
}
